...
@if(Session::has('password_changed'))
    <div class="alert alert-success" style="width: 50%;">{{ Session::get('password_changed') }}</div>
@endif
    
    <h1>Change Password</h1>
    {{-- button goes here for displaying add review --}}
    {{ Form::open(array('url' => 'changepassword', 'class' => 'form-signin')) }}
            @if (count($errors) > 0)
                @foreach ($errors->all() as $error)
                  <div class="alert alert-danger" style="width: 310px;">{{ $error }}</div>
                @endforeach
            @endif
            {{ Form::password('old_password', array('placeholder'=>'Old Password', 'class'=>'form-control', 'style' => 'width: 300px' ) ) }}
            {{ Form::password('password', array('placeholder'=>'New Password', 'class'=>'form-control', 'style' => 'width: 300px' ) ) }}
            {{ Form::password('password_confirmation', array('placeholder'=>'Confirm New Password', 'class'=>'form-control', 'style' => 'width: 300px' ) ) }}
        {{ Form::submit('Change Password', array('class' => 'btn btn-lg btn-primary btn-block btn-signin', 'style' => 'width: 215px')) }}
    {{ Form::close() }}
</div>
</body>
